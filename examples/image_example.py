...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.home_button = customtkinter.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="hooghoudt",
                                                   fg_color="transparent", text_color=("gray10", "gray90"), hover_color=("gray70", "gray30"),
                                                   image=self.chat_image, anchor="w", command=self.home_button_event)
        self.home_button.grid(row=1, column=0, sticky="ew")

        self.frame_2_button = customtkinter.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="Аверьянов",
                                                      fg_color="transparent", text_color=("gray10", "gray90"), hover_color=("gray70", "gray30"),
                                                      image=self.add_user_image, anchor="w", command=self.frame_2_button_event)
        self.frame_2_button.grid(row=2, column=0, sticky="ew")

        self.frame_4_button = customtkinter.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="Костиков",
                                                      fg_color="transparent", text_color=("gray10", "gray90"), hover_color=("gray70", "gray30"),
        ...
        #hogo
        self.second_frame_large_image_label = customtkinter.CTkLabel(self.home_frame, text="", image=self.hoogot_test_image,width=600)
        self.second_frame_large_image_label.grid(row=0, column=0, padx=20, pady=10)

        self.second_frame_button_1 = customtkinter.CTkButton(self.home_frame, text="Upload Excel File", image=self.image_icon_image, compound="right",width=600,height=60,command=self.upload_excel_file1)
        self.second_frame_button_1.grid(row=2, column=0, padx=20, pady=10)
        self.second_entry1 = customtkinter.CTkEntry(self.home_frame, placeholder_text="Path/To/Excel",width=600,height=50)
        self.second_entry1.grid(row=3, column=0, padx=20, pady=10)
        
        self.second_frame_button_2 = customtkinter.CTkButton(self.home_frame, text="Calculate", image=self.image_icon_image, compound="right",width=600,height=60,command=self.calculate_event1)
        ...
        #abechee
        self.home_frame_large_image_label = customtkinter.CTkLabel(self.second_frame, text="", image=self.agebt_test_image,width=600)
        self.home_frame_large_image_label.grid(row=0, column=0, padx=20, pady=20)
        
        self.home_frame_button_1 = customtkinter.CTkButton(self.second_frame, text="Upload Excel File", image=self.image_icon_image, compound="right",width=600,height=60,command=self.upload_excel_file2)
        self.home_frame_button_1.grid(row=2, column=0, padx=20, pady=10)
        self.home_entry1 = customtkinter.CTkEntry(self.second_frame, placeholder_text="Path/To/Excel",width=600,height=50)
        self.home_entry1.grid(row=3, column=0, padx=20, pady=10)

        self.home_frame_button_2 = customtkinter.CTkButton(self.second_frame, text="Calculate", image=self.image_icon_image, compound="right",width=600,height=60,command=self.calculate_event2)
        ...
        #kochoo
        self.forth_frame_large_image_label = customtkinter.CTkLabel(self.forth_frame, text="", image=self.kocho_test_image,width=600)
        self.forth_frame_large_image_label.grid(row=0, column=0, padx=20, pady=10)

        self.forth_frame_button_1 = customtkinter.CTkButton(self.forth_frame, text="Upload Excel File", image=self.image_icon_image, compound="right",width=600,height=60,command=self.upload_excel_file3)
        self.forth_frame_button_1.grid(row=2, column=0, padx=20, pady=10)
        self.forth_entry1 = customtkinter.CTkEntry(self.forth_frame, placeholder_text="Path/To/Excel",width=600,height=50)
        self.forth_entry1.grid(row=3, column=0, padx=20, pady=10)
        
        self.forth_frame_button_2 = customtkinter.CTkButton(self.forth_frame, text="Calculate", image=self.image_icon_image, compound="right",width=600,height=60,command=self.calculate_event3)
    ...
    def calculate_event1(self):
        try:
            ...
            df['de (cm) близкий(L=2500)s'] = np.where(((d/L_hogo) < 0.3) , (d/(1+((d/Lreal)*((8/np.pi)*np.log(d/r))-alfa))), '')
            df['L hooghoudt / У=L(real)'] = Lreal/L_hogo
            Y=df['L hooghoudt / У=L(real)']
...
#old
    ...
    def calculate_event3(self):
        try:
            ...
            #H
            wtd_cm = df['Water Table Depth, cm'] = pd.to_numeric(df['Water Table Depth, cm'], errors='coerce')
            #I
            ...
            # bbbb = df['B костиков, m']=(np.pi*k*daltaH)/(dringq*(np.log(bbbb/dm)-1))
            df['B костиков, m'] = 1.0
            iterations = 25
            for _ in range(iterations):
                s=df['B костиков, m'] = (np.pi * k * daltaH) / (dringq * (np.log(df['B костиков, m'] / dm) - 1))

            df['B костиков, m'] = s
            df['B real/B костиков']=(breal/df['B костиков, m'])
